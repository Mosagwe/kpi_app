...
import { authRoutes, initializeAuth, requireAdmin, requireAuth } from "./lib/auth.js";
...
import { getSettings, initializeSettings, removeLogo, saveLogo, saveSettings } from "./lib/settings.js";
import {
  closeDatabase,
  databaseStatus,
  getDatabase,
...
app.get("/favicon.ico", (_req, res) => {
  res.type("image/svg+xml").sendFile("public/favicon.svg", { root: process.cwd() });
});
...
app.put("/api/state", activeLicense, async (req, res) => {
...
app.post("/api/refine", activeLicense, async (req, res) => {
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  ...
  try {
    ...
    res.status(status).json({
      error: status === 401
        ...
        : status === 429
          ? "The OpenAI API project has no available quota or is being rate-limited. ChatGPT Plus does not cover API usage; check API billing, credits, and project limits."
...
app.post("/api/import", activeLicense, upload.single("workbook"), async (req, res) => {
...
app.post("/api/export", activeLicense, async (req, res) => {
...
app.post("/api/master/export", activeLicense, async (req, res) => {
  const { year = "master", kpis = [] } = req.body || {};
  if (!Array.isArray(kpis) || !kpis.length) {
    return res.status(400).json({ error: "There are no master KPIs to export." });
  }
  ...
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", `attachment; filename="${safeYear}-master-kpis.xlsx"`);
  res.send(buffer);
});
